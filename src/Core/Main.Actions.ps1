...
function NewVM(
    ...
    if ($Config) {
        $cfg = @{
            ...
            Path = if ($Config.Path) { "$($Config.Path)" } else { $script:Paths.VHD }
            ISO = if ($Config.ISO) { "$($Config.ISO)" } else { $null }
        }

        if ([string]::IsNullOrWhiteSpace($cfg.Name) -or $cfg.CPU -le 0 -or $cfg.RAM -le 0 -or $cfg.Storage -le 0) {
            Log "Invalid VM configuration. Name, CPU, RAM, and Storage must be provided." "ERROR"
            return $false
        }
    } else {
        if ($NonInteractive) {
            Log "Non-interactive VM creation requires a configuration object." "ERROR"
            return $false
        }
    ...
    $iso = $null
    if ($cfg.ISO -and (Test-Path $cfg.ISO)) {
        $useAutoInstall = $false
        $effectiveImageSelection = $ImageSelection
        $effectiveLocalAccountConfig = $LocalAccountConfig
        ...
        if ($NonInteractive) {
            ...
            if ($useAutoInstall) {
                $effectiveImageSelection = $null
                $effectiveLocalAccountConfig = $null
                $imageOptions = GetWindowsInstallImageOptions $cfg.ISO
                if ($imageOptions -and $imageOptions.Count -gt 0) {
                    $effectiveImageSelection = PromptWindowsInstallImageSelection $imageOptions
                } else {
                ...
                if (Confirm "Set local username/password in unattended setup?") {
                    ...
                    $effectiveLocalAccountConfig = [PSCustomObject]@{
        ...
        if ($useAutoInstall) {
            $iso = NewAutoISO $cfg.ISO $cfg.Name $effectiveImageSelection $effectiveLocalAccountConfig
...
function SetGPU($VMName=$null, $Pct=0, $GPUPath=$null, $GPUName=$null, [switch]$NonInteractive) {
    if (!$VMName) {
        if ($NonInteractive) {
            Log "VM name is required for non-interactive GPU partitioning." "ERROR"
            return $false
        }
        $vm = SelectVM "GPU PARTITION VM"; if (!$vm) { return $null }; $VMName = $vm.Name
    }
    if (!(Get-VM $VMName -EA SilentlyContinue)) { Log "VM not found" "ERROR"; Write-Host ""; return $false }
    if (!$GPUPath) {
        if ($NonInteractive) {
            Log "GPU path is required for non-interactive GPU partitioning." "ERROR"
            return $false
        }
...
function RemoveGPU($VMName=$null, $GPUPath=$null, [switch]$All, [switch]$CleanDrivers, [switch]$NonInteractive) {
    if (!$VMName) {
        if ($NonInteractive) {
            Log "VM name is required for non-interactive GPU removal." "ERROR"
            return $false
        }
        $vm = SelectVM "REMOVE GPU FROM VM"; if (!$vm) { return $null }; $VMName = $vm.Name
    }
    if (!(Get-VM $VMName -EA SilentlyContinue)) { Log "VM not found" "ERROR"; Write-Host ""; return $false }
    ...
    if ($NonInteractive) {
        if ($All -or [string]::IsNullOrWhiteSpace($GPUPath)) {
            $selectedAdapters = @($allAdapters)
            $selectedEntries = @($allEntries)
        } else {
            $targetKey = GetGpuIdentityKey $GPUPath
            for ($idx = 0; $idx -lt $allAdapters.Count; $idx++) {
                $adapter = $allAdapters[$idx]
                $adapterKey = GetGpuIdentityKey $adapter.InstancePath
                if (($targetKey -and $adapterKey -eq $targetKey) -or ($adapter.InstancePath -eq $GPUPath)) {
                    $selectedAdapters += $adapter
                    if ($idx -lt $allEntries.Count) { $selectedEntries += $allEntries[$idx] }
                }
            }
            if (!$selectedAdapters) {
                Log "No assigned GPU partition matched the provided GPU path." "ERROR"
                Write-Host ""
                return $false
            }
        }
        $cleanDrivers = $CleanDrivers.IsPresent
    } else {
        $items = @()
    ...
    $removeAll = ($selectedAdapters.Count -eq $allAdapters.Count)
    $selectedLabel = if ($removeAll) { "All GPU partitions" } elseif ($selectedEntries -and $selectedEntries.Count -gt 0) { $selectedEntries[0].Label } else { "Selected GPU partitions" }
...
function InstallDrivers($VMName=$null, $GPUPath=$null, [switch]$All, [switch]$SkipExisting, [switch]$NonInteractive) {
    if (!$VMName) {
        if ($NonInteractive) {
            Log "VM name is required for non-interactive driver injection." "ERROR"
            return $false
        }
        $vm = SelectVM "SELECT VM FOR DRIVERS"; if (!$vm) { return $null }; $VMName = $vm.Name
    }
    ...
    if ($NonInteractive) {
        if ($GPUPath) {
            ...
            foreach ($row in $adapterRows) {
                $adapterKey = GetGpuIdentityKey $row.Adapter.InstancePath
                if (($targetKey -and $adapterKey -eq $targetKey) -or ($row.Adapter.InstancePath -eq $GPUPath)) {
                    $selectedRows += $row
                }
            }
            if (!$selectedRows) {
...
function DeleteVM($VMName=$null, [switch]$DeleteFiles, [switch]$Force, [switch]$NonInteractive) {
    if (!$VMName) {
        if ($NonInteractive) {
            Log "VM name is required for non-interactive deletion." "ERROR"
            return $false
        }
        $vm = SelectVM "SELECT VM TO DELETE"; if (!$vm) { return $null }; $VMName = $vm.Name
    }
    ...
    if ($NonInteractive) {
        if (!$Force) {
            Log "Non-interactive deletion requires Force to avoid accidental VM removal." "ERROR"
            Write-Host ""
            return $false
        }
    } elseif (!(Confirm "Delete VM '$VMName'?")) {
        Log "Cancelled" "WARN"
        return $null
    }
